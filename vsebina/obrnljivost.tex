...
V tem razdelku predstavimo nekaj rezultatov iz homotopske teorije tipov, ki jih bomo v konstrukciji potrebovali. Ti so zelo standardni, zato se v njihovo motivacijo ne bomo poglabljali.
...
\begin{trditev}
  \label{sigma-assoc}
  Naj bo \(A\) tip, \(B\) družina tipov, indeksirana z \(A\) in \(C\) družina tipov, indeksirana z \(\typejudgment{x}{A}\) in \(B(x)\).
...
\begin{dokaz}
  ...
  velja ekvivalenca \(B(x) \simeq P(x) \times B(x)\).

  Naj bo torej \(\typejudgment{a}{A}\). Med tipoma konstruirajmo inverzni funkciji. Po predpostavki imamo družino funkcij
...
\end{trditev}
Z aplikacijo funkcije \(\proj{1}\) lahko za vsako identifikacijo tipa \(s = t\) dobimo tudi identifikacijo tipa \(\proj{1}(s) = \proj{1}(t)\), trditev pa nam pove, da lahko v primeru podtipov konstruiramo tudi enoličen dvig v nasprotno smer.
Trditev bi najlažje dokazali s tako imenovanim osnovnim izrekom o tipih identifikacij, vendar le tega v tem delu nismo razvili. Dokaz trditve in več o podtipih lahko poiščemo v \cite[Poglavje II.12.2]{rijke2022introduction}.
...
\begin{dokaz}
  ...
  Ker je po trditvi \ref{is-equiv-prop} \(\isequiv\) predikat na tipu \(A \to B\) in ker po trditvi \ref{inv-of-equiv} vsaka funkcija \(f\) porodi funkcija tipa
